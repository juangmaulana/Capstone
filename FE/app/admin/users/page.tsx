...
const USERS_COPY = {
  en: {
    stats: ["Total Users", "Active Users", "Admins", "New This Month"],
    title: "Users",
    addUser: "Add User",
    columns: ["Name", "Email", "Role", "Status", "Last Login"],
    roles: { Admin: "Admin", Researcher: "Researcher", Ranger: "Ranger" },
    status: { Active: "Active", Inactive: "Inactive" },
  },
  id: {
    stats: ["Total User", "User Aktif", "Admin", "Baru Bulan Ini"],
    title: "User",
    addUser: "Tambah User",
    columns: ["Nama", "Email", "Role", "Status", "Login Terakhir"],
    roles: { Admin: "Admin", Researcher: "Peneliti", Ranger: "Ranger" },
    status: { Active: "Aktif", Inactive: "Tidak Aktif" },
  },
} as const;

const statMeta = [
  { value: "24", icon: Users, color: "text-blue-600 bg-blue-50" },
  { value: "18", icon: Activity, color: "text-green-600 bg-green-50" },
  { value: "3", icon: ShieldCheck, color: "text-purple-600 bg-purple-50" },
  { value: "5", icon: UserPlus, color: "text-amber-600 bg-amber-50" },
];

export default function UserManagement() {
  const { language } = useLanguage();
  const copy = USERS_COPY[language];
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {statMeta.map((stat, index) => (
          <div key={copy.stats[index]} className="stat-card flex items-center gap-4">
            <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg ${stat.color}`}>
              <stat.icon className="h-5 w-5" />
            </div>
            <div>
              <p className="text-sm text-muted-foreground">{copy.stats[index]}</p>
                ...
                <tr key={user.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-medium ${
                      ...
                      {copy.status[user.status as keyof typeof copy.status] || user.status}
                    </span>
                  </td>
